 ...
 *
 * Three paths rather than one, and the reason is the waterline: it crosses the
 * document's outline and breaks it, so the page is drawn as the part above the
 * horizon and the part below it. Anything that recolours or strokes these has
 * to do it to all three or the page comes apart at the water.
 *
 * The paths are lifted verbatim from `assets/logo.svg`, which is the original
 * artwork and the source `scripts/make-icon.mjs` rasterises for the app icon.
 * Neither is edited by hand, so they cannot drift apart.
 */
...
const PATHS = [
  ...
  'm1026.43,919.62c.71,2.08,1.23,2.87,1.22,3.66-.11,43.19.57,86.41-.6,129.58-1.66,61.23-54.47,125.85-133.81,129.26-42.26,1.82-84.66.72-126.99.74-158.13.04-316.26-.02-474.38,0-34.22,0-65.55-8.92-92.53-30.48-34.9-27.89-53.53-64.18-54.01-109.1-.37-35.14-.06-70.28-.06-105.42,0-3.11,0-6.23,0-10.79,14,1.44,27,2.78,41.11,4.23,0,4.18,0,7.55,0,10.91,0,30.24,1.07,60.53-.24,90.72-2.42,56.02,46.4,109.6,108.68,109.38,151.21-.53,302.43-.18,453.65-.19,42.63,0,85.26-.03,127.88.02,64.05.07,110.71-46.52,110.75-110.66.02-32.55.39-65.1-.18-97.64-.15-8.61,2.6-11.87,10.76-12.21,9.66-.4,19.31-1.34,28.74-2.02Z',
];
...
export function PalmaMark({ size = 16, className }: { size?: number; className?: string }) {
  ...
  return (
    <svg
      className={['palmamark', className].filter(Boolean).join(' ')}
      width={size}
      height={size}
      viewBox={VIEW_BOX}
      fill="currentColor"
      stroke={weight > 0 ? 'currentColor' : undefined}
